...
class TestScopeConfig:
    ...
    def test_build_scope_path_nested_levels(self):
        ...
        path = config.build_scope_path({"org_id": "org_123", "user_id": "user_456"})
    ...
    def test_validate_scope_params_nested(self):
        ...
        config.validate_scope_params("user", {"org_id": "org_123", "user_id": "user_456"})
    ...
    def test_multiple_root_trees(self):
        ...
        org_tree = ScopeLevel("organization", "org_id", [ScopeLevel("user", "user_id")])
        tenant_tree = ScopeLevel("tenant", "tenant_id", [ScopeLevel("project", "project_id")])

        config = ScopeConfig([org_tree, tenant_tree])

        org_path = config.build_scope_path({"org_id": "org_123", "user_id": "user_456"})
        assert org_path == "organization:org_123/user:user_456"

        tenant_path = config.build_scope_path({"tenant_id": "tenant_123", "project_id": "proj_456"})
    ...
    def test_complex_hierarchy(self):
        ...
        path = config.build_scope_path({"org_id": "org_123", "user_id": "user_456", "session_id": "sess_789"})
        assert path == "organization:org_123/user:user_456/session:sess_789"

        config.validate_scope_params("session", {"org_id": "org_123", "user_id": "user_456", "session_id": "sess_789"})

        with pytest.raises(ValueError, match="Missing required parameter 'user_id'"):
            config.validate_scope_params("session", {"org_id": "org_123", "session_id": "sess_789"})
